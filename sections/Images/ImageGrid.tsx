...
function MediaItem({ item }: { item: Item }) { 
    ...
    return (
        <div class="w-1/3">
            <Image class="w-full h-full block sm:hidden rounded-lg" src={item.mobileImage!} alt={item.alt} width={item.width} height={item.height} />
            <Image class="w-full h-full hidden sm:block rounded-lg" src={item.image!} alt={item.alt} width={item.width} height={item.height} />
        </div>
    )
}
