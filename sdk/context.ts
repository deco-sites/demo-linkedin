...
let abort = () => { };
const enqueue = (cb: (signal: AbortSignal) => Promise<Partial<Context>> | Partial<Context>, options?: EnqueueOptions) => {
    abort();
    loading.value = true;
...
// Cart operations
const addToCart = async (item: AnalyticsItem, platformProps: PlatformCartProps) => {
    try {
        loading.value = true;
...
const updateQuantity = async (itemId: string, quantity: number) => {
    try {
        ...
        const cartData = await Runtime.invoke({
            ...
            props: {
                action: "set-quantity",
                items: Object.fromEntries(items.map((item, index) => [`item::${index}`, item.quantity])),
            },
        });
        context.cart.value = cartData as Minicart;
    }
